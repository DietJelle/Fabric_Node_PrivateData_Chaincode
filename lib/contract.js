...
class NodeContract extends Contract {
    ...
    async setPrivate(ctx, key) {
        // get the transient map
        let transientData = ctx.stub.getTransient();
        // read value from map (key is set by you)
        var data = transientData.get('privateData');
        await ctx.stub.putPrivateData("PrivateDataCollection", key, data);
    }

    async getPrivate(ctx, key) {
        let result = await ctx.stub.getPrivateData("PrivateDataCollection", key);
